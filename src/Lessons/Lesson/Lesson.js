...
const Lesson = (props) => {
	...
	if (lesson) {
		lessonMain = (
			<div>
				<h5 className="LessonHeaderBackground">{lesson.teacher.lastname} teaching {lesson.tune.lastname}</h5>
				...
				<p>Tune: <strong><Link to={`/tunes/${lesson.tune_id}`}>{lesson.tune.firstname} {lesson.tune.lastname}</Link></strong></p>
